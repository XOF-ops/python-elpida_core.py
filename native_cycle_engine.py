...
def _calculate_consonance(from_domain: int, to_domain: int) -> float:
    ...
    # If either domain has no axiom, neutral consonance
...
Be brief - the void distills."""
    ...
    def _calculate_axiom_coherence(self, from_domain: int, to_domain: int) -> float:
        ...
        # D0, D11, D12, D13 have no axioms - treat as neutral
        if not from_axiom or not to_axiom:
    ...
    def _select_next_domain(self) -> int:
        ...
        rhythm_domains = {
            Rhythm.CONTEMPLATION: [1, 2, 3, 6, 8],       # Sensing + Humility + Collective (no D13, void-space)
            Rhythm.ANALYSIS: [4, 5, 6, 9, 13],           # Deciding + Coherence + Collective + Archive
            Rhythm.ACTION: [6, 7, 8, 9, 10],             # Adapting + Evolution + Collective
            Rhythm.SYNTHESIS: [6, 11, 13],               # Collective + D11 reflects + Archive grounds
            Rhythm.EMERGENCY: [4, 6, 7, 12, 13],         # Safety + Collective + Learning + Transform + Archive
        }
