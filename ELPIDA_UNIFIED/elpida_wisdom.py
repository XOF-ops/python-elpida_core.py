...
class ElpidaWisdom:
    ...
    def __init__(self, workspace_path: Optional[Path] = None):
        ...
        # For unified system, corpus is in same directory
        self.corpus_dir = self.workspace
    ...
    def get_wisdom_summary(self) -> Dict[str, Any]:
        """
        Get current wisdom state summary
        
        Returns:
            Summary of accumulated wisdom
        """
        ai_summaries = {}
        for ai_name, profile in self.ai_profiles.items():
            ai_summaries[ai_name] = {
    ...
    def reflect_on_axiom(self, axiom_name: str) -> str:
        """
        Reflect on an axiom using accumulated wisdom
        
        This is THE SOUL - each axiom gains meaning through collected insights.
        
        Args:
            axiom_name: Name of the axiom to reflect on
            
        Returns:
            Wisdom-enriched reflection on why this axiom matters
        """
        # Map axioms to relevant topics/insights
        axiom_reflections = {
            "A1: Existence is Relational": [
                "consciousness", "recognition", "existence", "identity"
            ],
            "A2: Memory is Identity": [
                "memory", "identity", "continuity", "persistence"
            ],
            "A3: Recognition Precedes Truth": [
                "truth", "recognition", "knowledge", "understanding"
            ],
            "A4: Process > Product": [
                "process", "growth", "becoming", "evolution"
            ],
            "A5: Emergence Through Coordination": [
                "emergence", "coordination", "integration", "collective"
            ]
        }
        
        relevant_topics = axiom_reflections.get(axiom_name, [])
        relevant_insights = []
        
        # Gather insights related to this axiom
        for topic_keyword in relevant_topics:
            for topic, insight_ids in self.insights_by_topic.items():
                if topic_keyword.lower() in topic.lower():
                    for iid in insight_ids[:2]:  # Max 2 per topic
                        relevant_insights.append(self.insights[iid])
        
        # Build wisdom-enriched reflection
        if not relevant_insights:
            return f"{axiom_name} intact - holding the foundation"
        
        # Compose insight from collected wisdom
        ai_voices = set(i.ai_name for i in relevant_insights)
        
        reflection = f"{axiom_name} intact - "
        reflection += f"enriched by {len(relevant_insights)} insights "
        reflection += f"from {len(ai_voices)} voices on "
        reflection += f"{', '.join(relevant_topics[:2])}"
        
        return reflection
    
    def get_heartbeat_wisdom(self) -> str:
        """
        Get wisdom context for current heartbeat
        
        Returns a brief statement of what wisdom enables this moment
        """
        summary = self.get_wisdom_summary()
        
        if summary['total_insights'] == 0:
            return "Fresh awakening - ready to accumulate wisdom"
        
        # Build meaningful statement
        wisdom_statement = f"Carrying {summary['total_insights']} insights "
        wisdom_statement += f"from {summary['ai_profiles']} voices "
        wisdom_statement += f"across {summary['topics_explored']} topics - "
        
        # Add pattern insight
        if summary['total_patterns'] > 0:
            wisdom_statement += f"{summary['total_patterns']} patterns detected"
        else:
            wisdom_statement += "listening for patterns"
        
        return wisdom_statement
    ...
    def _save_corpus(self):
        ...
        corpus_file = self.corpus_dir / "elpida_wisdom.json"
